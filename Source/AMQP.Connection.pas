...
{$ENDIF}
  ...
  Public
    Constructor Create(AConnection: IAMQPConnection; ATCP: TIdTcpClient; AMainQueue: TAMQPQueue; AChannelList: TThreadList<IAMQPChannel>; ADumpMethod: TDumpMethod; ADumpHeader: TDumpHeader; ADumpFrame: TDumpFrame);
  End;
...
begin
  ...
  Try
    ...
    Method.Field['client-properties'].AsFieldTable.Add('platform', TLongString.Create(PlatformID)).Add('application', TLongString.Create(ApplicationID)).Add('hostname', TLongString.Create(Hostname))
      .Add('capabilites', TFieldTable.Create.Add('publisher_confirms', True).Add('exchange_exchange_bindings', True).Add('basic,nack', True).Add('consumer_cancel_notify', True).Add('queue-mode', 'lazy, default').Add('exchange_type', 'topic, headers, fanout, direct'));
...
begin
  ...
  if not MethodIsExpected then
    raise AMQPException.CreateFmt('Unexpected class/method: %d.%d', [result.Payload.AsMethod.ClassID.Value, result.Payload.AsMethod.MethodID.Value]);
end;
...
constructor TAMQPThread.Create(AConnection: IAMQPConnection; ATCP: TIdTcpClient; AMainQueue: TAMQPQueue; AChannelList: TThreadList<IAMQPChannel>; ADumpMethod: TDumpMethod; ADumpHeader: TDumpHeader; ADumpFrame: TDumpFrame);
begin
